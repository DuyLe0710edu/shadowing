...
import { AppleTranslationManager } from './AppleTranslationManager'
...
export class TranslationManager {
  ...
  private appleTranslationManager: AppleTranslationManager
  private translationWorker: Worker | null = null
  private isWorkerReady: boolean = false
  private pendingRequests: Map<string, { resolve: Function, reject: Function }> = new Map()
  private requestCounter: number = 0
  private readonly ENABLE_FLOATING_TRANSLATION_OVERLAY = false
  ...
  constructor(ocrServiceManager: OCRServiceManager) {
    ...
    this.appleTranslationManager = new AppleTranslationManager()
        ...
        try {
          const languageSettings = await new Promise((resolve) => {
            ...
            if (mainWindow) {
              ...
              const handler = (event: any, settings: any) => {
          ...
          sourceLang = (languageSettings as any).source || 'auto'
          targetLang = (languageSettings as any).target || 'en'
        } catch (error) {
  ...
  public async translateText(text: string, sourceLang: string = 'auto', targetLang: string = 'en'): Promise<TranslationResult> {
