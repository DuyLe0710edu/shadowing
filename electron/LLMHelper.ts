...
export class LLMHelper {
  ...
  public async generateVocabCards(pairs: Array<{ originalText: string; translation: string }>, sourceLang: string, targetLang: string, limit: number = 30) {
    try {
      const trimmed = pairs
      ...
      console.warn('[LLMHelper] generateVocabCards fallback due to error:', (error as any)?.message)
      const fallback = pairs.slice(0, limit).map((p, idx) => this.fallbackVocabFromPair(p, idx))
      return fallback
    }
  }

  private fallbackVocabFromPair(pair: { originalText: string; translation: string }, idx: number) {
    ...
    if (!front || front.length > 40) front = src.slice(0, 40)
    let back = tgt
    if (back.length > 80) back = back.slice(0, 80)
