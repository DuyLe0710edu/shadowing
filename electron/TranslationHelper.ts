...
export interface PinyinResult {
  ...
  pinyin: string
  tones: number[]
  syllables: { text: string, pinyin: string, tone: number }[]
}
...
export class TranslationHelper {
  ...
  private readonly SYSTEM_PROMPT = `You are a specialized translation assistant focused on language learning, particularly Mandarin Chinese. 
...
Format your response as JSON:
{
  ...
  private async initializeOCR(): Promise<void> {
    try {
      this.ocrWorker = await createWorker('eng+chi_sim+chi_tra', 1, {
  ...
  public async extractTextFromImage(imagePath: string): Promise<{ text: string, confidence: number }> {
    if (!this.ocrWorker) {
  ...
  public async translateText(text: string, targetLanguage: string = 'en', sourceLanguage?: string): Promise<TranslationResult> {
    // Check cache first
  ...
  private async translateWithGemini(text: string, targetLanguage: string, sourceLanguage?: string): Promise<TranslationResult | null> {
...
Remember to include Pinyin for any Chinese text and maintain natural language flow for subtitles.`
  ...
  private async translateWithGoogle(text: string, targetLanguage: string, sourceLanguage?: string): Promise<TranslationResult | null> {
    try {
      // Note: This would require Google Translate API key
      // For now, implementing a placeholder that could be extended
      const response = await axios.post('https://translate.googleapis.com/translate_a/single', {
        client: 'gtx',
      ...
      if (response.data && response.data[0] && response.data[0][0]) {
        ...
        let pinyinResult: PinyinResult | undefined
  ...
  private async translateOffline(text: string, targetLanguage: string, sourceLanguage?: string): Promise<TranslationResult | null> {
  ...
  private async generatePinyin(chineseText: string): Promise<PinyinResult | undefined> {
    try {
  ...
  public getProviderStatus(): TranslationProvider[] {
